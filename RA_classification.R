# Red alder classification project with UW

# I want to be able to have the folder with all the plots in google drive
# so I can access the folder as a drive. Initially I couldn't get this drive
# link to show up on my desktop so I had Ally share the folder to my UW email.
# this seemed to fix the problems.

# There are 2 files for the trees. The first has the "turning point" tree (this is
...
clipCrownCircle <- function (
  ...
  radius,
  lastReturns = FALSE,
  inputFiles = "G:/temp/Keven_polys/DNR_Portal_Data/LAS/*.las",
  outputFolder = "G:/temp/Keven_polys/tempclips/",
  groundFile = "G:/temp/Keven_polys/DNR_Portal_Data/LAS/split_dtm/sappho_grnd_*.dtm"
) {
...
computeMetrics <- function (
  ...
  coverThreshold = 4.5,
  firstReturns = TRUE,
  inputFiles = "G:/temp/Keven_polys/tempclips/*.las",
  outputFolder = "G:/temp/Keven_polys/tempclips/",
...
#  plotID <- "B1E3_TP_25_100"     # testing only
  ...
  trees <- mergeTrees(plotID, dataFolder, logCon = logCon)
...
#  mapview(trees, zcol = "Species")
  ...
  # compute metrics
  res <- computeMetrics(plotID, deleteClips = TRUE)
  
  # read metrics and extract highest point in the sample
  metrics <- read.csv(paste0(tempClipFolder, plotID, "_", tempMetricsFileBase), stringsAsFactors = FALSE)
  ...
  trees$TrueRadius <- (0.28 * trees$LidarHt - 6.62) * radiusFudgeFactor
  
  # condition the radius...radius equation goes negative for heights < 23.64 feet
  ...
  res <- computeMetrics(plotID, deleteClips = FALSE)
  ...
  res <- computeMetrics(plotID, firstReturns = FALSE, deleteClips = TRUE)
  ...
  res <- computeMetrics(plotID, heightThreshold = 0.0, firstReturns = FALSE, deleteClips = TRUE)
...
dataFolder <- "G:/R_Stuff/RAClassification/Georeferenced_tree_plots_2020_sampling/"
...
tempClipFolder <- "G:/temp/Keven_polys/tempclips/"
...
for (i in 1:length(plotList)) {
  processPlot(plotList[i], assumedHeight = 37, logCon = logCon)
}
...
# 8/13/2021...these values are wrong...I think they were computed before I constrained the 
# radius to be >= 2 feet
# median value of TrueRadius is 2.842 feet. this works out to a circular
# area ~25 sq ft. I used 5 foot cells for area-wide metrics
# mean value for TrueRadius is 3.379 feet...this would better match 6 foot cells
#
# 8/13/2021...these are the correct values:
# median value of TrueRadius is 3.718 feet. this works out to a circular
# area ~43 sq ft (6.6 foot cell size). I used 5 foot cells for area-wide metrics
# mean value for TrueRadius is 3.978 feet...this would better match 7 foot cells


# drop things directly related to height or highly correlated with height
# This is only needed for modeling with all variables. For the final models,
# a small subset of variables was used and explicitly passed to randomForest().

# Also drop variables that we know are not useful like return counts, min and max (affected
# by outliers), and the tree-related columns. You could do this in the call to randomForest
# but I think it is easier to do this explicitly so we have a clean data set to work with.

# this was for the "original" model
#trainData <- trainData[, -c(1, 3:62, 102:103, 110:115)]
# this one includes some of the low height metrics
# column 2 is the Species
# column 143 is the SpeciesGroup
#trainData <- trainData[, -c(1, 3:33, 53:62, 102:103, 110:115, 143)] # by Species
#trainData <- trainData[, -c(1, 2, 3:33, 53:62, 102:103, 110:115)]   # by SpeciesGroup...only low elev

# after some testing for the alder/conifer classifier, it looks like the elevation metrics
# don't cause any problems. They don't show up in the list by variable importance scores
# until #32 (P01) and #35 (P20). This selector leaves all elevation metrics in the set
...
# write off RF model
if (modelType == "ALRU") {
  saveRDS(typeRF, "FINAL_RF_AlderModel.rds")
} else {
  saveRDS(typeRF, "FINAL_RF_DW_WH_model.rds")
}
...
if (TRUE & modelType == "ALRU") {
  ...
  saveRDS(result, "Sensitivity_RF_AlderModel.rds")

  result <- result[order(result[, "accuracy"], decreasing = TRUE), ]
  result
}
result <-   readRDS("Sensitivity_RF_AlderModel.rds")
